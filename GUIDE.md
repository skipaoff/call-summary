...
1. **Источник записей — Notion, Zoom или оба сразу:**
   ...
   - **Только Telegram (эконом, без Notion/Zoom)** — если коннекторов нет вообще: просто
     **пересылай транскрипты созвонов боту** текстом в течение дня, а вечером рутина
     соберёт из них сводку. Ничего подключать не нужно, кроме самого бота.
...
Источник: ВПИШИ notion, zoom, telegram ИЛИ all.
Если notion или all — моя страница записей: ВСТАВЬ_ССЫЛКУ_NOTION.
...
3. Достань встречи за сегодня (date -u) из выбранного источника:
   ...
   - telegram: node scripts/telegram-inbox.mjs <дата> — прочитай пересланные боту
     транскрипты, каждое сообщение = один звонок;
   - all: возьми из всех источников и объедини в один список (дубли не склеивай).
