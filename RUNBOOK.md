...
### 1. Конфиг
Прочитать `config.json` → `source.notionPageId`, `site.vercelProject`, `site.vercelScope`.

### 2. Достать встречи за сегодня (адаптер Notion)
- `notion-fetch` по `notionPageId` → список дочерних страниц-встреч.
- Отобрать те, чья дата в заголовке = сегодня (сверить с `date -u`).
- Для каждой: `notion-fetch` с `include_transcript=true` → берём `<transcript>`.

> Смена источника = переписать только этот шаг. Дальше всё работает с единым форматом.
