...
### 1. Конфиг
Прочитать `config.json` → `source.type` (`notion` | `zoom` | `telegram` | `all`),
...
**telegram** (эконом-режим, без Notion/Zoom): пользователь днём пересылает боту
транскрипты текстом; вечером считываем их:
- `node scripts/telegram-inbox.mjs <YYYY-MM-DD>` → `{date, messages:[{time,text}]}`.
- Каждое сообщение = один звонок (`text` — транскрипт). Бот-приёмник = тот же
  (`TELEGRAM_BOT_TOKEN`), без вебхука.
- Работает на Routines без постоянного сервера: Telegram держит апдейты до 24ч, вечерний
  прогон забирает всё за день. Условие — переслать до прогона; лимит чтения 100 сообщений.

**all**: выполнить все настроенные ветки и объединить встречи в один `meetings[]`,
отсортировать по времени. Дубли НЕ склеиваем — показываем все версии (у каждой свой `source`).
