...
- **Источник:** `config.json` → `source.type` = `notion` | `zoom` (что выбрал юзер).
...
2. **Достань встречи за сегодня — по `source.type`** (адаптер; всё остальное ниже
   ...
   **Если `zoom` (Zoom-коннектор):**
   - Через Zoom-коннектор получи **облачные записи** пользователя за сегодня
     (`date -u`; нужен тариф Zoom Pro+ с включёнными cloud recording и audio
     transcription — иначе транскрипта нет).
   - Для каждой записи возьми **транскрипт** (файл типа `audio_transcript`, формат
     VTT) и преобразуй в чистый текст: убери таймкоды/нумерацию строк, склей реплики.
   - `title` = тема встречи, `time` = время начала, `participants` = участники, если
     коннектор их отдаёт.
   - Если у записи транскрипт ещё не готов (Zoom обрабатывает его отдельным этапом) —
     пропусти её на сегодня (заберётся следующим прогоном).
