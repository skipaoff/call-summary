...
- **Источник:** `config.json` → `source.type` = `notion` | `zoom` | `both` (что выбрал юзер).
...
2. **Достань встречи за сегодня — по `source.type`** (адаптер; всё остальное ниже
   ...
   **Если `both`:** выполни ОБЕ ветки (Notion и Zoom) и объедини все встречи в один
   `meetings[]`. **Дубли не склеиваем** — если один созвон попал и в Notion, и в Zoom,
   показываем обе версии (у каждой проставлен свой `source`, на странице это видно).
   Отсортируй встречи по времени.
